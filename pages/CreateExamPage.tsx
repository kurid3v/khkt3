...
import CalendarPicker from '../components/CalendarPicker'; // New import
import CalendarIcon from '../components/icons/CalendarIcon'; // New import

interface CreateExamPageProps {
  onAddExam: (title: string, description: string, startTime: number, endTime: number, password?: string) => void;
  onCancel: () => void;
}

const CreateExamPage: React.FC<CreateExamPageProps> = ({ onAddExam, onCancel }) => {
  const [title, setTitle] = useState('');
  const [description, setDescription] = useState('');
  const [password, setPassword] = useState('');
  const [error, setError] = useState('');

  // Use Date objects for state
  const now = new Date();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setError('');
    onAddExam(title, description, startTimeMs, endTimeMs, password.trim() || undefined);
  };
