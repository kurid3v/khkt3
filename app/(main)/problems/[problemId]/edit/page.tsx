...
export default function EditProblemPage({ params }: { params: { problemId: string } }) {
    ...
    const { problems, updateProblem } = useDataContext();
    ...
    if (!problem) {
        return <p className="p-8">Loading or problem not found...</p>;
    }
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        updateProblem(updatedProblemData);
        if (problem.examId) {
            router.push(`/exams/${problem.examId}`);
        } else {
            router.push('/admin');
        }
    };
